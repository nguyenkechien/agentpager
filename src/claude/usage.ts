...
import { z } from 'zod';
import type { UsageReport, UsageSource, UsageWindow } from '../sessions/limits.js';
...
export function parseUsageResponse(raw: unknown): UsageReport {
  ...
  for (const [key, label] of PLAN_WINDOWS) {
    ...
    if (!window) continue;
    windows.push({ key, label, utilizationPercent: percent(window.utilization), resetsAtMs: isoToMs(window.resets_at) });
  }
...
export class SdkUsageSource implements UsageSource {
  constructor(
    private readonly deps: { claudeExecutable: string; cwd: string; logger: Logger; timeoutMs?: number },
  ) {}
  ...
  async fetch(): Promise<UsageReport> {
    ...
    const usageQuery = query({
      prompt: emptyInput,
      options: { pathToClaudeCodeExecutable: this.deps.claudeExecutable, cwd: this.deps.cwd, settingSources: [] },
    });
    const drained = (async () => {
