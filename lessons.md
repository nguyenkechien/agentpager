...
- A workflow job without `permissions: contents: write` cannot see draft releases: `gh release view <tag>` answers "release not found" even though the draft exists. The finalize check of the first release (v0.1.0) failed only for that reason; the same check run with the owner's login passed.
- A silent NSIS uninstall (`/S`) relaunches itself from a temporary folder and the started process exits at once; it removes the window profile, then program files, then shortcuts. Poll for each effect; a check right after the program files disappear raced with the shortcut removal.
