...
describe('trayModel', () => {
  it('shows a running bot in green with Stop and Restart', () => {
    expect(trayModel(view('running', 'test_bot'))).toEqual({
      color: 'green',
      tooltip: 'agentpager — Running · @test_bot',
      statusLine: 'Running · @test_bot',
      items: [
  ...
  it('colours transitions amber and unreachable daemons red, keeping Stop available', () => {
    ...
    expect(trayModel(view('restarting')).statusLine).toBe('Restarting');
    expect(trayModel(view('unresponsive'))).toMatchObject({ color: 'red', statusLine: 'Bot not responding' });
    expect(trayModel(view('disconnected')).items.map((item) => item.action)).toEqual(['open', 'stop', 'restart', 'quit']);
    expect(trayModel(view('running')).statusLine).toBe('Running');
  });
  ...
  it('offers a downloaded update or a newer release before Quit', () => {
    ...
    expect(ready.items.slice(-2)).toEqual([
      { action: 'update', label: 'Update to v0.1.1', enabled: true },
      { action: 'quit', label: QUIT_LABEL, enabled: true },
    ]);
    ...
    expect(available.items.at(-2)).toEqual({ action: 'update', label: 'Download v0.2.0', enabled: true });
  ...
  it('waits for the first status read', () => {
    expect(trayModel(null)).toMatchObject({ color: 'grey', statusLine: 'Reading status…' });
